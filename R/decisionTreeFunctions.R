...
mergeEdgeTransition <- function(cohortData,
  ...
  dTreeEdges <-
    dplyr::left_join(
      dTreeEdges,
      stateTransitionTable,
      by = c("from_label" = "FROM_LABEL", "to_label" = "TO_LABEL")
    )
  ...
  if (probabilityType == 1) {
    ...
    summarydTreeEdges <-
      dplyr::summarise(.data = summarydTreeEdges, PROB = PROB / sum(PROB))
    ...
    g <-
      igraph::graph_from_data_frame(data.frame(dTreeEdges$from, dTreeEdges$to))
    
    l <-
      lapply(igraph::V(g), function(x)
        igraph::all_shortest_paths(g, from = dTreeEdges$from[1]))
    l <- lapply(l, function(x)
      x[[-2]])
    l <- unlist(l, recursive = F, use.names = F)
    paths <-
      lapply(1:(nrow(dTreeEdges) + 1), function(x)
        igraph::as_ids(l[[x]]))
    paths <-
      lapply(1:(nrow(dTreeEdges) + 1), function(n)
        plyr::mapvalues(
          x = paths[[n]],
    ...
    for (patientId in unique(cohortData_copy$SUBJECT_ID)) {
      ...
      trajectories$COUNT[stringr::str_detect(fullTrajectorie, trajectories$TRAJECTORIE)] <-
        trajectories$COUNT[stringr::str_detect(fullTrajectorie, trajectories$TRAJECTORIE)] + 1
      options(warn = defaultW)
    }
    trajectories$PROB <-
      trajectories$COUNT / length(unique(cohortData_copy$SUBJECT_ID))
