...
kmDataPreparation = function(patientData, startCohortId, endCohortId, ageInterval = NULL, selectedIntervals = NULL, survivalType = "regular") {
  if(is.null(patientData$AGE)){patientData$AGE = 0}
  patientData$SUBJECT_ID = as.integer(patientData$SUBJECT_ID)
  if("COHORT_START_DATE" %in% colnames(patientData)){
    colnames(patientData) = c("SUBJECT_ID", "STATE", "STATE_START_DATE", "STATE_END_DATE", "GENDER_CONCEPT_ID", "AGE", "TIME_IN_COHORT")
  }
  data = dplyr::select(patientData, SUBJECT_ID, STATE, STATE_START_DATE, STATE_END_DATE, AGE)
  ...
  if(nrow(data1) == 0 | nrow(data2) == 0)
    ...
    colnames(df) <- c("SUBJECT_ID", "START_DATE", "AGE", "ID_START", "END_DATE", "ID_END", "OUTCOME", "GROUP", "DATE")
    return(df)
  }
  ...
  if(survivalType == "nearest"){
    data2 = dplyr::mutate(
      dplyr::summarise(
        dplyr::group_by(
          data2,
          SUBJECT_ID),
        END_DATE = min(STATE_START_DATE), ID = min(ID)),
      STATE = endCohortId)
    
    
    data_surv =  merge(data1, data2, by = "SUBJECT_ID", all.x = T)
    data_surv = dplyr::select(data_surv, -STATE_END_DATE, -ID.x, -ID.y)
    
    colnames(data_surv) <- c("SUBJECT_ID", "ID_START", "START_DATE", "AGE", "END_DATE", "ID_END")
    data_surv = dplyr::mutate(
      dplyr::filter(
        data_surv, START_DATE < END_DATE | is.na(END_DATE)), DIFF = difftime(START_DATE,END_DATE))
    data_surv = dplyr::filter(
      dplyr::group_by(
          data_surv,
          SUBJECT_ID),
    ...
    data_surv = dplyr::slice(dplyr::group_by(data_surv, SUBJECT_ID),1)
    # ids = c()
  ...
  data1 = dplyr::mutate(
    dplyr::summarise(
      dplyr::group_by(
        ...
        SUBJECT_ID),
      # We are taking min(AGE) so that the inclusion age of the patient could be used in analysis
      STATE_START_DATE = min(STATE_START_DATE), AGE = min(AGE), ID = min(ID)),
    STATE = startCohortId)
  ...
  data_surv = dplyr::mutate(data_surv, OUTCOME = ifelse(is.na(END_DATE),0,1))
  data_surv$GROUP = "All"
  ################################################################################
  #
  ...
  if (!(is.null(ageInterval))){
    ...
    for (chosenInterval in selectedIntervals){
      data_clone_to_add = dplyr::filter(dplyr::mutate(data_clone, GROUP = dplyr::if_else(as.numeric(AGE) >= chosenInterval[1] & as.numeric(AGE) <= chosenInterval[2],
                                                                                         paste(chosenInterval[1],chosenInterval[2], sep = "-") , GROUP)),
                                        GROUP != "All")
      data_surv = rbind(data_surv, data_clone_to_add)
    }
  }

  data_surv = dplyr::mutate(data_surv, DATE = ifelse(is.na(END_DATE),-1,as.numeric(difftime(END_DATE, START_DATE, units = "days"))))
  # When the difference of dates is negative, switch outcome to 0?
  data_surv = dplyr::mutate(data_surv, OUTCOME = ifelse(DATE < 0,0,OUTCOME))
  max_date = max(data_surv$DATE)
  data_surv = dplyr::mutate(data_surv, DATE = ifelse(DATE == -1,max_date ,DATE))
